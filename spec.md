...
`answer`는 오타 단어(플레이어가 찾아야 할 단어)이고, `options`는 정상 단어와 오타 단어가 혼합된 배열이다.

`stages.ts` 내부의 `makeOptions` 헬퍼로 배열을 생성한다.
...
export const stages: Stage[] = [
  {
    id: 1,
    answer: '샤자',                         // 오타 단어
    options: makeOptions('사자', '샤자', 4), // 정상 3개 + 오타 1개
  },
  {
    id: 2,
    answer: '바따',
...
`options` 배열은 런타임에 셔플되어 오타 위치가 매 플레이마다 달라진다. (`src/shared/utils/shuffle.ts`)
...
`Stage.answer`는 오타 단어다. 플레이어가 오타 단어를 클릭하면 정답이다.
...
* 현재 단계 표시
* 남은 시간을 Progress Bar로 표시
* 단어 그리드 표시 (정상 단어 여러 개 + 오타 단어 1개)

그리드 레이아웃:

* `cols = Math.round(Math.sqrt(options.length))` 로 열 수를 결정한다.
* Stage 1(4장): 2×2, Stage 2(9장): 3×3, Stage 3(16장): 4×4, Stage 4~5(25장): 5×5
* 버튼 크기는 사용 가능한 영역을 cols/rows로 나눠 자동 계산한다.
* 오타 위치는 `TypoTrapGame.showPlaying()`에서 `shuffle(options)`로 랜덤화된다.
...
```txt
[ ] 시작 화면이 보인다.
[ ] 시작 버튼 클릭 시 1단계(2×2 그리드)가 시작된다.
[ ] 3초 타이머 Progress Bar가 감소한다.
[ ] 그리드에 정상 단어 여러 개와 오타 단어 1개가 표시된다.
[ ] 오타 단어의 위치가 매 플레이마다 랜덤하게 바뀐다.
[ ] 오타 단어 클릭 시 정답 화면이 나온다.
[ ] 다음 버튼 클릭 시 다음 단계로 이동한다.
[ ] 단계별 그리드 크기가 커진다 (2×2 → 3×3 → 4×4 → 5×5).
[ ] 정상 단어 클릭 시 게임 오버가 나온다.
[ ] 시간 초과 시 게임 오버가 나온다.
[ ] 다시 시작 버튼이 동작한다.
